...
// Fetch 10 proyek terbaru dari GitHub
let projectsData = [];

async function fetchLatestProjects() {
  try {
    const response = await fetch(
      "https://api.github.com/users/fajarjulyana/repos?sort=updated&per_page=10"
    );
    const repos = await response.json();

    // Filter & format data untuk ditampilkan
    projectsData = repos.map((repo) => ({
      id: repo.name,
      titleKey: repo.name,
      descKey: repo.description || "Tidak ada deskripsi",
      tech: [],
      categories: [],
    ...
    // Render ulang proyek setelah data diambil
    renderProjects();
  } catch (error) {
          ...
          <li class="skill-item">
            <div class="skill-info">
              <span class="skill-name">${
                t[skill.nameKey] || skill.nameKey
              }</span>
              <span class="skill-level">${skill.level}%</span>
            </div>
            <div class="skill-bar">
              <div class="skill-progress" style="width: 0%" data-level="${
                skill.level
              }"></div>
            </div>
          </li>
